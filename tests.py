...
class TestColorfulCurry(unittest.TestCase):
    ...
    def test_pipeline_with_reduce(self):
        ...
        styles = [WHITE, BG_MAGENTA, BOLD]
        style = reduce(lambda a, b: a(b), styles) 
        result = style("combined")
        expected = f"{AnsiStyle.WHITE.value}{AnsiStyle.BG_MAGENTA.value}{AnsiStyle.BOLD.value}combined{AnsiStyle.RESET.value}"
        self.assertEqual(result, expected)
    ...
    def test_alternate_string_arg_location(self):
        # Test that the style works when the string is not the first argument
        def log_message(level, message):
            print(f"{level}: {message}")

        captured_output = io.StringIO()
        sys.stdout = captured_output
        RED(log_message,str_idx=1)("INFO", "This is a test")
        sys.stdout = sys.__stdout__
        result = captured_output.getvalue()
        expected = f"INFO: {AnsiStyle.RED.value}This is a test{AnsiStyle.RESET.value}"
        expected = captured_output.getvalue()
        self.assertEqual(result, expected)
